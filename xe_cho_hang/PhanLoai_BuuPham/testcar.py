...
def findContour(img, spa=200, f=0):
    ...
    if f:
        cv.imshow('imh', erode)
    contours = cv.findContours(erode, cv.RETR_TREE, cv.CHAIN_APPROX_NONE)[0]
    ...
    for i in range(n_boxes):
        if int(d['conf'][i]) > 60:
            ...
            if text and text.strip() != "":
                ...
                img = cv.putText(img, text, (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3)
                st = True
                word = text
    return st, img, word
...
def stackImages(img, img1, labels, sizeWork=0.5, size=(680, 480)):
    ...
    if len(labels) > 0:
        for i, label in enumerate(labels):
            img1 = cv.putText(img1, str(label), ((int(size[0] // 5) * (i + 1)), int(size[1] // 4 * 3.7)), 2, sizeWork,
                              (0, 50, 255), 2)
    else:
        img1 = cv.putText(img1, '[Nothing!]', (int(size[0] // 2.2), int(size[1] // 4 * 3.7)), 2, sizeWork, (0, 0, 255),
                          2)
...
if __name__ == "__main__":
    ...
    labels = 'DT-TN SC-TN PY-TN PB-TN COMMON'
    add = 1
    print(labels)
    word_ = 'Unknow'
    time_ = [0, 0, 0]
